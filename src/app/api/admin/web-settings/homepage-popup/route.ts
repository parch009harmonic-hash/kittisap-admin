...
import {
  getWebHomepagePopupSettingsApi,
  updateWebHomepagePopupSettingsApi,
...
function mapStatus(message: string) {
  ...
  if (message.startsWith("Missing homepage popup columns")) return 503;
...
export async function GET() {
  try {
    ...
    const data = await getWebHomepagePopupSettingsApi();
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load homepage popup settings";
    return NextResponse.json(
      { ok: false, code: "WEB_HOMEPAGE_POPUP_FETCH_FAILED", error: message },
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebHomepagePopupSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    if (isUiMaintenanceLockedError(error)) {
      return NextResponse.json(
        { ok: false, code: "UI_MAINTENANCE_LOCKED", error: error.message },
        { status: 423 },
      );
    }
    const message = error instanceof Error ? error.message : "Failed to update homepage popup settings";
    return NextResponse.json(
      { ok: false, code: "WEB_HOMEPAGE_POPUP_UPDATE_FAILED", error: message },
      { status: mapStatus(message) },
    );
  }
}
