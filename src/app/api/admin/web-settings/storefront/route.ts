...
import {
  getWebStorefrontSettingsApi,
  updateWebStorefrontSettingsApi,
...
export async function GET() {
  try {
    ...
    const data = await getWebStorefrontSettingsApi();
    ...
    return NextResponse.json({ ok: false, code: "WEB_STOREFRONT_FETCH_FAILED", error: message }, { status: mapStatus(message) });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebStorefrontSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Failed to update storefront settings";
    return NextResponse.json({ ok: false, code: "WEB_STOREFRONT_UPDATE_FAILED", error: message }, { status: mapStatus(message) });
  }
}
