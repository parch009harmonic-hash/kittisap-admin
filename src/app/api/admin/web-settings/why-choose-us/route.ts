...
import {
  getWebWhyChooseUsSettingsApi,
  updateWebWhyChooseUsSettingsApi,
...
export async function GET() {
  try {
    ...
    const data = await getWebWhyChooseUsSettingsApi();
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load why choose us settings";
    return NextResponse.json({ ok: false, code: "WEB_WHY_CHOOSE_US_FETCH_FAILED", error: message }, { status: mapStatus(message) });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebWhyChooseUsSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Failed to update why choose us settings";
    return NextResponse.json({ ok: false, code: "WEB_WHY_CHOOSE_US_UPDATE_FAILED", error: message }, { status: mapStatus(message) });
  }
}
