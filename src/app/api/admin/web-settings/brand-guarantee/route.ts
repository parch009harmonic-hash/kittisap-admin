...
import {
  getWebBrandGuaranteeSettingsApi,
  updateWebBrandGuaranteeSettingsApi,
...
export async function GET() {
  try {
    ...
    const data = await getWebBrandGuaranteeSettingsApi();
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load brand guarantee settings";
    return NextResponse.json({ ok: false, code: "WEB_BRAND_GUARANTEE_FETCH_FAILED", error: message }, { status: mapStatus(message) });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebBrandGuaranteeSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Failed to update brand guarantee settings";
    return NextResponse.json({ ok: false, code: "WEB_BRAND_GUARANTEE_UPDATE_FAILED", error: message }, { status: mapStatus(message) });
  }
}
