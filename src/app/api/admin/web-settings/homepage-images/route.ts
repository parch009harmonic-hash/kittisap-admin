...
import {
  getWebHomepageImageStripSettingsApi,
  updateWebHomepageImageStripSettingsApi,
...
export async function GET() {
  try {
    ...
    const data = await getWebHomepageImageStripSettingsApi();
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load web homepage image settings";
    return NextResponse.json({ ok: false, code: "WEB_HOMEPAGE_IMAGES_FETCH_FAILED", error: message }, { status: mapStatus(message) });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebHomepageImageStripSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Failed to update web homepage image settings";
    return NextResponse.json({ ok: false, code: "WEB_HOMEPAGE_IMAGES_UPDATE_FAILED", error: message }, { status: mapStatus(message) });
  }
}
