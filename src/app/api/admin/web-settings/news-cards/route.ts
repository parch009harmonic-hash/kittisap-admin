...
import {
  getWebNewsCardsSettingsApi,
  updateWebNewsCardsSettingsApi,
...
export async function GET() {
  try {
    ...
    const data = await getWebNewsCardsSettingsApi();
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load news cards settings";
    return NextResponse.json({ ok: false, code: "WEB_NEWS_CARDS_FETCH_FAILED", error: message }, { status: mapStatus(message) });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const data = await updateWebNewsCardsSettingsApi(payload);
    return NextResponse.json({ ok: true, data }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    ...
    const message = error instanceof Error ? error.message : "Failed to update news cards settings";
    return NextResponse.json({ ok: false, code: "WEB_NEWS_CARDS_UPDATE_FAILED", error: message }, { status: mapStatus(message) });
  }
}
